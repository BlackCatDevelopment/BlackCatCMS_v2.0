...
<body class="fc_gradient1">
	...
	<header id="fc_admin_header" class="fc_gradient1 fc_border">
		...
		<nav>
			<ul id="fc_menu">
				{foreach $MAIN_MENU as menu}{if $menu.permission_title != 'preferences'}
				<li>
					<a href="{$menu.link}" target="_self" class="icon-fc_{$menu.permission_title}{if $menu.current == true} fc_current{/if}">{$menu.title}</a>
				</li>
				{else}{$preferences = $menu}{/if}{/foreach}
			</ul>
		</nav>
        {if is_array($preferences)}{* user/displayname and link to preferences *}
		<div id="fc_account">
