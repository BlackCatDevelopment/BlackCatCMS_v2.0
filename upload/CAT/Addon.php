...
if (!class_exists('CAT_Addon', false))
{
	class CAT_Addon extends CAT_Object
	{
		...
		public static function setIDs($section_id=NULL)
		{
			if( !(self::$page_id && self::$section_id) )
			{
				...
				if (!$section_id )	global $section_id;
		...
		{
			$path		= CAT_PATH . '/modules/' . static::$directory . '/inc/headers/' . self::getVariant() .'/'.$for.'.ini';
